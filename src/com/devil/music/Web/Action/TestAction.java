...
public class TestAction extends BasicAction {
	...
	public static void main(String[] args){
		...
		List<Term> list = HanLP.segment("我爱周杰伦，你爱我");
		for(Term t:list){
			String w = t.word;
	...
	void saveRank(){
		...
		if (recreate) {
			getCollection("posting").drop();
			getDatabase().createCollection("posting");
		}
		MongoCollection<Document> col_index = getCollection("posting");
	
		IndexOptions indexOptions = new IndexOptions().unique(true);
		col_index.createIndex(Indexes.ascending("name"), indexOptions);
		
		File file = new File(AppSettings.INDEX_PATH);
		if (!file.exists()) {
			System.out.println("Error: can't find index file.");
		}
		...
		try {
			...
			while (true) {
				...
				ArrayList<Document> posting = new ArrayList<Document>();
				for(String s:list){
					String[] tmp = s.split(":");
					if(isFirst){
						isFirst = false;
						doc.append("name", tmp[0]);
						doc.append("nums", Integer.parseInt(tmp[1]));
						doc.append("list", posting);
					}
					else{
						Document p = new Document();
						p.append("id", Integer.parseInt(tmp[0]));
						p.append("nums", Integer.parseInt(tmp[1]));
						posting.add(p);
					}
				}
				//System.out.println(doc.toString());
				col_index.insertOne(doc);
				nums++;
			}
			bf.close();
			System.out.println("Create database success!");
			System.out.println("Number of indexes insert: " + nums);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
